...
class GumbelFeatureSelection(nn.Module):
    ...
    def forward(self, lid, src_feat, trg_feat):
        ...
        if not self.training and len(relevance) == 1:
    ...
    def _softmax(self, soft_sample):
        ...
        hard_sample = soft_sample.detach().clone().zero_().scatter(dim=1, index=hard_sample_idx, src=torch.ones_like(hard_sample_idx).cuda().float())
